 ...
 */
export function formatEvaluatorNames(evaluators) {
 ...
 */
export function formatEvaluatorExamples(evaluators) {
  return evaluators
    .map((evaluator) => {
 ...
 */
export function formatEvaluators(evaluators) {
  return evaluators.map((evaluator) => `'${evaluator.name}: ${evaluator.description}'`).join(',\n');
}
export const evaluatorsProvider = {
  name: 'EVALUATORS',
