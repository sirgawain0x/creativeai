...
function getSalt(runtime) {
  const secretSalt =
    (typeof process !== 'undefined' ? process.env.SECRET_SALT : import.meta.env.SECRET_SALT) ||
    'secretsalt';
 ...
 */
export async function updateWorldSettings(runtime, serverId, worldSettings) {
  try {
 ...
 */
export async function getWorldSettings(runtime, serverId) {
  try {
    const worldId = createUniqueUuid(runtime, serverId);
    const world = await runtime.getWorld(worldId);
 ...
 */
export async function initializeOnboarding(runtime, world, config) {
  try {
