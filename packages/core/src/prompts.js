 ...
 */
export const composePrompt = ({ state, template }) => {
 ...
 * @returns {string} The composed prompt output.
 */
export const composePromptFromState = ({ state, template }) => {
  const templateStr = typeof template === 'function' ? template({ state }) : template;
  const templateFunction = handlebars.compile(templateStr);
 ...
 */
export const addHeader = (header, body) => {
 ...
 */
export const composeRandomUser = (template, length) => {
...
export const formatPosts = ({ messages, entities, conversationHeader = true }) => {
  ...
  const formattedPosts = sortedRooms.map(([roomId, roomMessages]) => {
    const messageStrings = roomMessages
      ...
      .map((message) => {
        const entity = entities.find((entity) => entity.id === message.entityId);
 ...
 */
export const formatMessages = ({ messages, entities }) => {
  const messageStrings = messages
    ...
    .map((message) => {
      ...
      const formattedName =
        entities.find((entity) => entity.id === message.entityId)?.names[0] || 'Unknown User';
      ...
      const actionString =
        messageActions && messageActions.length > 0
          ? `${textString ? '' : timestampString} (${formattedName}'s actions: ${messageActions.join(', ')})`
          : null;
...
export const formatTimestamp = (messageDate) => {
  const now = new Date();
 ...
 */
export function parseBooleanFromText(value) {
 ...
 */
export function extractAttributes(response, attributesToExtract) {
  const attributes = {};
...
export const parseActionResponseFromText = (text) => {
  const actions = {
    like: false,
    retweet: false,
    quote: false,
 ...
 */
export function truncateToCompleteSentence(text, maxLength) {
...
export async function splitChunks(content, chunkSize = 512, bleed = 20) {
 ...
 */
export async function trimTokens(prompt, maxTokens, runtime) {
